...
impl<T: Default + Clone> DoublyLinkedList<T> {
    pub fn new() -> Self {
    ...
    pub fn push_front(&mut self, key: T) -> Rc<RefCell<Node<T>>> {
    ...
    pub fn push_back(&mut self, key: T) -> Rc<RefCell<Node<T>>> {
    ...
    pub fn remove_node(&mut self, node: Rc<RefCell<Node<T>>>) {
        let next_link = node.borrow().next.clone().unwrap();
        let prev_link = node.borrow().prev.clone().unwrap();
        next_link.borrow_mut().prev = Some(prev_link.clone());
        prev_link.borrow_mut().next = Some(next_link.clone());
    ...
    pub fn push_node_front(&mut self, node: Rc<RefCell<Node<T>>>) {
        let old_first_link = self.head.borrow().next.clone();

        node.borrow_mut().next = old_first_link.clone();
        self.head.borrow_mut().next = Some(node.clone());
        if let Some(ref first) = old_first_link {
    ...
    pub fn peek_front(&mut self) -> Option<Rc<RefCell<Node<T>>>> {
        if let Some(ref next) = self.head.borrow().next.clone() {
    ...
    pub fn peek_back(&mut self) -> Option<Rc<RefCell<Node<T>>>> {
        if let Some(ref prev) = self.tail.borrow().prev.clone() {
            if Rc::ptr_eq(prev, &self.head) {
                return None;
            }
        }
...
mod doubly_linked_lists_tests {
    ...
    fn test_remove() {
        ...
        list.push_back(1); // [1]
        list.push_front(2); // [2, 1]
        let last = list.push_back(3); // [2, 1, 3]
        list.remove_node(last); // [2,1]
        assert_eq!(list.pop_front().unwrap().borrow().key, 2);
        assert_eq!(list.pop_back().unwrap().borrow().key, 1);
        assert!(list.pop_front().is_none());
    }

    #[test]
    fn test_push_node_front() {
        let mut list = DoublyLinkedList::new();
        list.push_back(1); // [1]
        list.push_front(2); // [2, 1]
        let last = list.push_back(3); // [2, 1, 3]
        list.remove_node(last.clone()); // [2,1]
        list.push_node_front(last);

        assert_eq!(list.pop_front().unwrap().borrow().key, 3);
        assert_eq!(list.pop_back().unwrap().borrow().key, 1);
        assert_eq!(list.pop_back().unwrap().borrow().key, 2);
        assert!(list.pop_front().is_none());
    }
    ...
    fn test_peek_single_element() {
        ...
        assert_eq!(front.borrow().key(), 10);
        assert_eq!(back.borrow().key(), 10);
    }

    #[test]
    fn test_peek_two_elements() {
        let mut list = DoublyLinkedList::new();
        ...
        assert_eq!(front.borrow().key(), 2);
        assert_eq!(back.borrow().key(), 1);
    }
    ...
    fn test_peek_multiple_elements() {
        ...
        assert_eq!(front.borrow().key(), 4);
        assert_eq!(back.borrow().key(), 1);
    }

    #[test]
    fn test_peek_after_removing_front() {
        let mut list = DoublyLinkedList::new();
    ...
    #[test]
    fn test_peek_after_removing_back() {
        let mut list = DoublyLinkedList::new();
        ...
        list.remove_node(n1.clone());

        let back = list.peek_back().unwrap();
        assert_eq!(back.borrow().key(), 2);
    }
}
